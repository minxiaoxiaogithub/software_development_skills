...
- 项目初始化：
  - 根据技术方案初始化项目，包含基础配置和依赖安装
  - 检查是否已有 `.gitignore`，若没有则根据技术栈生成
  - 创建 `.env.example`，区分 dev/staging/prod，说明每个变量用途，敏感值不提交仓库
- 公共模块：识别公共模块，输出接口规范，确认后在本阶段优先开发；必须包含使用文档，web 项目需包含统一演示入口页面
...
- 公共模块：清单 + 接口规范 + 使用文档（web 项目含演示入口页面）

## Review 检查清单

- 技术方案：已确认
- 任务拆解：每个任务可独立开发和 review
- 公共模块：清单完整，接口规范已输出，有使用文档（web 项目有演示入口页面）
